...
function withPreload(
  factoryOrModules: PageFactory | PageModules
): WithPreloadResult {
  const relativeBase = base === '' || base.startsWith('.');

  function enhanceFactory(factory: PageFactory): WithPreloadResult {
    const links = factory
      .toString()
      .match(/('|")(.*?)\1/g)
      ?.map(link => {
  ...
  function enhanceModules(modules: PageModules): WithPreloadResult {
    return {
      preload() {
        return modules;
      },
...
export function generateWithPreloadCode(viteConfig: ResolvedConfig) {
  return `const seen = {};
const base = '${viteConfig.base}';
const assetsDir = '${viteConfig.build.assetsDir}';
${withPreload.toString()}`;
}
